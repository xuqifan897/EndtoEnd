...
void
RTStruct::setSubCntrPoint( unsigned int r, unsigned int s, unsigned int p, float v_x, float v_y, float v_z )
{
    if (p == 0 && s == 0)
    {
...
float3
RTStruct::getSubCntrPoint( unsigned int r, unsigned int s, unsigned int p )
{
    float3 point;
...
bool
RTStruct::loadDicomInfo(bool verbose)
{
...
void
RTStruct::loadRTStructInfo(bool verbose)
{
...
int
RTStruct::loadRTStructData( int r, bool verbose)
{
    DRTStructureSet dcmrt_struct;
    OFCondition status = dcmrt_struct.loadFile( dicom_full_filename.data() );
    if (status.good())
...
void
RTStruct::saveRTStructData( const char *outpath, ROI_DATA *new_roi_data, unsigned int new_roi_count, bool anonymize_switch )
{
    DcmFileFormat format;
    OFCondition status = format.loadFile( dicom_full_filename.data() );
    if (status.bad())
