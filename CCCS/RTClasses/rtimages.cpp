...
bool RTImage::loadDicomInfo(bool verbose) {
...
bool RTImage::importSOPClassUID(const std::string& fname ) {
    char *filename;
...
void RTImage::importImagePositionPatient( unsigned int i )
{
    DcmFileFormat format;
...
void RTImage::importInstanceNumber( unsigned int i ) {
    DcmFileFormat format;
    OFCondition status = format.loadFile( slice[i].filename.data() );
    if (status.good())
    {
...
void RTImage::importPatientInfo() {
    DcmFileFormat format;
...
int RTImage::loadRTImageData(bool verbose, bool flipXY) {
    data_min = 9999;
    data_max = -9999;
...
int RTImage::saveRTImageData(const std::string& outpath, float *newData, bool anonymize_switch ) {
...
void RTImage::saveRTImageData(const std::string& outpath, bool anonymize_switch ) {
    for (unsigned int i=0; i<image_count; i++)
    {
        DcmFileFormat format;
...
void RTImage::anonymize(DcmDataset *dataset) {
