...
__device__ float3
d_inverseRotateKernelAtOriginRHS(const float3& vec, const float& theta, const float& phi)
{
    float3 roll = d_rotateAroundAxisAtOriginRHS(vec, make_float3(0.f, 1.f, 0.f), phi);               // kernel roll
    float sptr, cptr;
    fast_sincosf(phi, &sptr, &cptr);
    return d_rotateAroundAxisAtOriginRHS(roll, make_float3(cptr, 0.f, -sptr), theta);
}

__device__ float3
d_inverseRotateBeamAtOriginRHS(const float3& vec, const float& theta, const float& phi, const float& coll)
{
...
__device__ float3
d_inverseRotateBeamletAtOriginRHS( const float3& vec, const float& theta, const float& phi) {
    float sptr, cptr;
    fast_sincosf(-phi, &sptr, &cptr);
    float3 rotation_axis = make_float3(sptr, 0.0f, cptr); // couch rotation
    return d_rotateAroundAxisAtOriginRHS(vec, rotation_axis, -theta);     // gantry rotation
}

__device__ float3 
d_rotateKernelAtOriginRHS( const float3& vec, const float& theta, const float& phi ) {
    float sptr, cptr;
    fast_sincosf(phi, &sptr, &cptr);
    float3 untilt = d_rotateAroundAxisAtOriginRHS(vec, make_float3(cptr, 0.f, -sptr), -theta);
    return d_rotateAroundAxisAtOriginRHS(untilt, make_float3(0.f, 1.f, 0.f), -phi);
}

__device__ float3
d_rotateBeamletAtOriginRHS( const float3& vec, const float& theta, const float& phi) {
    return d_inverseRotateBeamletAtOriginRHS(vec, -theta, phi);
}
...
)
{
    ...
    if (subvoxidx == 0) {
        ...
        // some uses of the axes will thus be transposed
        {
            ...
            float3 pbev_idx = d_inverseRotateKernelAtOriginRHS(__fmaf_rn(revStart, __frcp_rn(rev_voxelsize), make_float3(tZ, tX, tY)), kern_theta, kern_phi)+0.5f;
    ...
    if (CHECK_ACTIVE_THREAD) {
        ...
        if (subvoxidx == 0) {
            ...
            g_coords = d_inverseRotateBeamAtOriginRHS(
                    d_inverseRotateBeamletAtOriginRHS(in_pillar_idx*rev_voxelsize, beamletAngles.x, beamletAngles.y),
...
__global__ void
old::PackRowConvolve(float *bDens,
    float *bTerma,
...
)
{
    ...
    float3 rev_idx = __fmaf_rn(-revStart, __frcp_rn(rev_voxelsize), d_rotateKernelAtOriginRHS(make_float3(pX, pY, pZ), kern_theta, kern_phi));
...
UnpackBEVDosePillar (
    ...
    /*                    beam_azimuth, beam_zenith, beam_coll */
    /*                )/rev_voxelsize + 0.5f; */
    float3 p_idx = d_rotateBeamletAtOriginRHS(
                       d_rotateBeamAtOriginRHS(g_coords-pillarStartCoords, beam_azimuth, beam_zenith,beam_coll),
