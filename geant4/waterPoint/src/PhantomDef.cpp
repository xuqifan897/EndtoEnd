...
void wp::GeomDef::display()
{
    ...
    float photonCountsTotal = 0;
    for (int i=0; i<Spec.size(); i++)
    {
        photonCounts[i] = std::get<1>(Spec[i]) / std::get<0>(Spec[i]);
        photonCountsTotal += photonCounts[i];
    }
    ...
    for (int i=0; i<Spec.size(); i++)
    {
        std::get<2>(Spec[i]) = int(std::round(
            photonCounts[i] / photonCountsTotal * nParticles));
    ...
    std::cout << std::setw(20) << std::left << "Energy (MeV)" << std::setw(20) << 
        std::left << "Energy Fluence" << std::setw(20) << std::left << "Number Fluence" << 
...
std::vector<std::tuple<float, float, int, int>> wp::Spec{
    {0.20, 0.001, 0, 0},
    {0.30, 0.010, 0, 0},
    {0.40, 0.020, 0, 0},
    {0.50, 0.030, 0, 0},
