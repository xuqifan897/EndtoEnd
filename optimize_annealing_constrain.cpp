...
void optimize(vector<beam>& beams, phantom& Phtm, FCBBkernel* kernel, float** h_loss, float** h_smoothness_loss, \
    float** h_perturbation_energy, float** h_zenith, float** h_azimuth, float** h_probability, bool** h_taken)
{
    ...
    vector<float> zenith_range = get_args<vector<float>>("zenith-range");
    uint random_seed = get_args<uint>("random-seed");
    srand(random_seed);
    ...
    if (*h_perturbation_energy == nullptr)
        *h_perturbation_energy = (float*)malloc(iterations*beams.size()*NUM_PERTURBATIONS*sizeof(float));
    if (*h_zenith == nullptr)
    ...
    if (*h_probability == nullptr)
        *h_probability = (float*)malloc(iterations*beams.size()*sizeof(float));
    if (*h_taken == nullptr)
        *h_taken = (bool*)malloc(iterations*beams.size()*sizeof(bool));
    ...
    for (int iter=0; iter<iterations; iter++)
    {
        ...
        for (int i=0; i<beams.size(); i++)
        {
            ...
            beams[i].zenith = beams[i].zenith + (random01() - 0.5) * step_size_angular;
            beams[i].azimuth = beams[i].azimuth + (random01() - 0.5) * step_size_angular;
            // clamp the zenith value to make it within the zenith range
            if (beams[i].zenith < zenith_range[0])
                beams[i].zenith = zenith_range[0];
            else if (beams[i].zenith > zenith_range[1])
                beams[i].zenith = zenith_range[1];
            (*h_zenith)[(iter * beams.size() + i) * NUM_PERTURBATIONS + 1] = beams[i].zenith;
            (*h_azimuth)[(iter * beams.size() + i) * NUM_PERTURBATIONS + 1] = beams[i].azimuth;
            beams[i].BEV_dose_forward(Phtm, kernel);
            beams[i].PVCS_dose_forward(Phtm);
            dose_sum(beams, Phtm, &d_PVCS_total_dose, d_sources);
            beam::calc_FCBB_PVCS_dose_grad(Phtm, &d_element_wise_loss, d_PVCS_total_dose);
            reduction(d_element_wise_loss, phantom_size, d_out0, perturbation_loss, 1);

            // compare
            float* h_perturbation_energy_pointer = (*h_perturbation_energy) + (iter * beams.size() + i) * NUM_PERTURBATIONS;
            checkCudaErrors(cudaMemcpy(h_perturbation_energy_pointer, perturbation_loss, \
                NUM_PERTURBATIONS*sizeof(float), cudaMemcpyDeviceToHost));
            float probability;
            if (h_perturbation_energy_pointer[1] <= h_perturbation_energy_pointer[0])
                probability = 1.;
            else
                probability = exp((h_perturbation_energy_pointer[0] - h_perturbation_energy_pointer[1]) / current_temperature);
            (*h_probability)[iter * beams.size() + i] = probability;
            if (random01() <= probability)
            {
                (*h_taken)[iter * beams.size() + i] = true;
            }
            else
            {
                (*h_taken)[iter * beams.size() + i] = false;
                beams[i].zenith = (*h_zenith)[(iter * beams.size() + i) * NUM_PERTURBATIONS];
                beams[i].azimuth = (*h_azimuth)[(iter * beams.size() + i) * NUM_PERTURBATIONS];
                beams[i].BEV_dose_forward(Phtm, kernel);
                beams[i].PVCS_dose_forward(Phtm);
            }
        }
        ...
        cout << "Iteration: " << iter + 1 << ", step size: " << step_size << \
            ...
            ", smoothness loss: " << total_smoothness_loss << ", step taken: ";
        for (int i=0; i<beams.size(); i++)
            cout << (*h_taken)[iter * beams.size() + i];
        cout << endl;
    }
...
void log_out(vector<beam>& beams, phantom& Phtm, FCBBkernel* kernel, float* h_loss, float* h_smoothness_loss, \
    float* h_perturbation_loss, float* h_zenith, float* h_azimuth, float* h_probability, bool* h_taken)
{
    ...
    outFile.close();
    cout << "Perturbation loss written to " << lossOutputPath << endl;
    ...
    outFile.close();
    cout << "zenith angle written to " << lossOutputPath << endl;
    ...
    outFile.close();
    cout << "azimuth angle written to " << lossOutputPath << endl;

    lossOutputPath = output_folder + "/probability.dat";
    outFile.open(lossOutputPath);
    if (! outFile.is_open())
    {
        cout << "Could not open probability file: " << lossOutputPath << endl;
        exit(EXIT_FAILURE);
    }
    outFile.write((char*)h_probability, get_args<int>("iterations")*beams.size()*sizeof(float));
    outFile.close();
    cout << "Probability written to " << lossOutputPath << endl;

    lossOutputPath = output_folder + "/taken.dat";
    outFile.open(lossOutputPath);
    if (! outFile.is_open())
    {
        cout << "Could not open taken output file: " << lossOutputPath;
        exit(EXIT_FAILURE);
    }
    outFile.write((char*)h_taken, get_args<int>("iterations")*beams.size()*sizeof(bool));
    outFile.close();
    cout << "Taken action written to " << lossOutputPath << endl;
...
int main(int argc, char** argv)
{
    ...
    float* h_smoothness_loss = nullptr;
    float* h_perturbation_energy = nullptr;
    float* h_zenith = nullptr;
    float* h_azimuth = nullptr;
    float* h_probability = nullptr;
    bool* h_taken = nullptr;
    optimize(beams, Phtm, kernel, &h_loss, &h_smoothness_loss, \
        &h_perturbation_energy, &h_zenith, &h_azimuth, &h_probability, &h_taken);
...
int main_arithmetic_test(int argc, char** argv)
{
    ...
    for (int i=0; i<input_log.size(); i++)
        cout << "(" << input_log[i] << ", " << output_log[i] << ")" << endl;
    
    vector<float> input_exp{1., 1.5, 2., 3.};
    vector<float> output_exp(input_exp.size(), 0);
    for (int i=0; i<input_exp.size(); i++)
        output_exp[i] = exp(input_exp[i]);
