...
for idx = 1:numPatients
    ...
    optFolder = fullfile(patExpFolder, 'optimizePTVcropped');
...
%     save(MorgFile, 'M', 'dose_data', 'masks', '-v7.3');
    maskFile = fullfile(optFolder, 'masks.mat');
    save(maskFile, 'dose_data', 'masks', '-v7.3');

    % save parameter files
    [StructureInfo, params] = getParams(M, dose_data, masks, PrescriptionDose);
    ParamsNum = 0;
    paramsPath = fullfile(optFolder, ['params', num2str(ParamsNum), '.mat']);
...
function [StructureInfo, params] = getParams(M,dose_data,masks,pdose)
