...
h5file = fullfile(patFolder,'Dose_Coefficients.h5');
maskfile = fullfile(patFolder,'Dose_Coefficients.mask');
thresh = 1e-06;
[M,dose_data,masks]=BuildDoseMatrix(h5file, maskfile, thresh);
save(fullfile(patFolder,[patientName '_M_original.mat']),'M','dose_data','masks','-v7.3');

% save parameter files
[StructureInfo, params] = InitIMRTparams(M,dose_data,masks,PresriptionDose);
ParamsNum = 0;
...
M = M(:,newbeamletind);
save(fullfile(patFolder,[patientName '_M.mat']),'M','-v7.3');
