function sliceCallBack_QL(instr, varargin)
...
%             hQuest = questdlg('Do you want to save changes made to this plan?','Saving plan','Yes','No','No');
    ...
    case 'DOSETOGGLE'
        ...
        try
            ...
            if stateS.doseToggle == 1 || strcmpi(varargin{1},'on')
    ...
    case 'SLIDERTRANSALPHA'
        %figure(hCSV); %Remove uicontrol focus.
        if(~isempty(varargin))
            stateS.doseAlphaValue.trans = varargin{1};
        else
    ...
    case 'TOGGLESINGLESTRUCT'
        ...
        SelectStruct = varargin{1};
        for i = 1:length(SelectStruct)
            planC{indexS.structures}(SelectStruct(i)).visible = 1;
        end
        stateS.structsChanged = 1;
        CERRRefresh
...
%             end
    ...
    case 'CTWINDOW'
        ...
        set(stateS.handle.CTPreset, 'Value', 1);
        CTlevel = varargin{1}(1);
        CTwidth = varargin{1}(2);
        
        scanSet = stateS.handle.aI(stateS.currentAxis).scanSets;
        scanSet = scanSet(1);
        
        stateS.optS.CTLevel = CTlevel;
        stateS.optS.CTWidth = CTwidth;
...
%         stateS.scanStats.Colormap.(scanUID) = stateS.optS.scanColorMap(colorMapIndex).name;
    ...
    case 'SELECTAXISVIEW'
        ...
        if(length(varargin)>2)
            ...
            [~, ~ , coord] = getAxisInfo(uint8(stateS.currentAxis), 'view', 'scanSets', 'coord');
        end
        setAxisInfo(hAxis, 'coord', coord, 'view', view, 'xRange', [], 'yRange', []);
        updateAxisRange(hAxis,0)
        setAxisInfo(uint8(stateS.currentAxis), 'coord', coord);
