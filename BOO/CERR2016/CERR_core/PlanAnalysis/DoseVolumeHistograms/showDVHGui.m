...
switch upper(command)
    case 'INIT'
        ...
        %Set up add frame
        afX = 10; afW = w - 2*afX; afH = 300; afY = h - 10 - afH;
        ...
        ud.af.handles.dose = uicontrol(hFig, 'style', 'listbox','Max',10,'Min',1, 'units', units, 'position',  [afX+10+55 afY + 0 110 300] , 'string', doseList, 'value', initialDoseNum, 'horizontalAlignment', 'left', 'callback', 'showDVHGui(''DOSEPICKED'')','Tag','doseSelectTag');
        ...
        ud.af.handles.structure = uicontrol(hFig, 'style', 'listbox','Max',10,'Min',1, 'units', units, 'position',[afX+75+110+55 afY + 0 120 300], 'string', strList, 'horizontalAlignment', 'left','Tag','structSelectTag');
    ...
    case 'ADDDVH'
        ...
        for glass=1:numel(doseIndex)
            for gorilla=1:numel(strNum)
            
            strName = planC{indexS.structures}(strNum(gorilla)).structureName;
            nDVH = length(planC{indexS.DVH});
            planC{indexS.DVH}(nDVH+1).structureName = strName;
            planC{indexS.DVH}(nDVH+1).assocStrUID=planC{indexS.structures}(strNum(gorilla)).strUID;
            planC{indexS.DVH}(nDVH+1).dvhUID = createUID('dvh');
            planC{indexS.DVH}(nDVH+1).doseIndex = doseIndex(glass);
            planC{indexS.DVH}(nDVH+1).assocDoseUID = planC{indexS.dose}(doseIndex(glass)).doseUID;
            planC{indexS.DVH}(nDVH+1).dateOfDVH = date;
            planC{indexS.DVH}(nDVH+1).fractionIDOfOrigin = planC{indexS.dose}(doseIndex(glass)).fractionGroupID;
