...
{
    int iterations_per_stage = get_args<int>("iterations");
    int iterations = iterations_per_stage * 3;
    ...
    *h_loss = (float*)malloc(iterations*sizeof(float));
    ...
    for (int iter = 0; iter<iterations; iter++)
    {
        ...
        if (iter % iterations_per_stage == 0 && iter != 0)
...
void log_out(vector<beam>& beams, phantom& Phtm, FCBBkernel* kernel, float* h_loss)
{
    ...
    for (int idx=0; idx<beams.size(); idx++)
    {
        ...
        outputPath_ss << output_folder << "/fluence" << setfill('0') << setw(3) << idx + 1 << ".dat";
